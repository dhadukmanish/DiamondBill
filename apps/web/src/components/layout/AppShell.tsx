...
import { Bell, ChevronDown, ChevronRight, Gem, LogOut, Menu, MessageSquare, PanelLeftClose, PanelLeftOpen, Search, Settings, User } from 'lucide-react';
import { NAV, type NavItem } from '@diamondbill/shared';
  ...
  return (
    <aside className={cx('flex h-full flex-col border-r border-line bg-white transition-all', collapsed ? 'w-[68px]' : 'w-[260px]')}>
      <Link to="/dashboard" className="flex h-16 items-center gap-2.5 border-b border-line px-4">
        <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary text-white"><Gem className="h-5 w-5" /></span>
        {!collapsed && (
          <div className="min-w-0">
            <div className="font-heading text-[15px] font-semibold text-gray-900 leading-tight">DiamondBill</div>
            <div className="truncate text-[11px] text-gray-500">{tenantName || 'Diamond Finance'}</div>
          </div>
        )}
      </Link>
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-60 card shadow-lg py-1">
          ...
          <button onClick={() => { setOpen(false); nav('/modules/settings/users'); }} className="flex w-full items-center gap-2 px-4 py-2 text-[13px] text-gray-700 hover:bg-gray-50"><User className="h-4 w-4" /> My Profile</button>
          ...
          <button onClick={async () => { try { await api.post('/api/auth/logout'); } catch {} logout(); nav('/signin'); }} className="flex w-full items-center gap-2 px-4 py-2 text-[13px] text-red-600 hover:bg-gray-50"><LogOut className="h-4 w-4" /> Sign out</button>
        </div>
      )}
    </div>
  );
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-16 shrink-0 items-center gap-3 border-b border-line bg-white px-4 sm:px-6">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button className="icon-btn rounded-full" title="Messages"><MessageSquare className="h-4 w-4" /></button>
            <button className="icon-btn rounded-full relative" title="Notifications"><Bell className="h-4 w-4" /><span className="absolute -top-0.5 -right-0.5 h-2 w-2 rounded-full bg-red-500" /></button>
            <UserMenu />
          </div>
        </header>
