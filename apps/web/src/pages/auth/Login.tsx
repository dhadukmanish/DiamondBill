...
export default function Login() {
  const { accessToken, setTokens, setTenantName } = useAuthStore();
  ...
  const submit = handleSubmit(async (v) => {
    ...
    try {
      ...
      setTenantName(r.user.tenantName ?? '');
  ...
  return (
    <div className="flex min-h-screen bg-page">
      <div className="hidden lg:flex w-1/2 flex-col justify-between bg-gradient-to-br from-primary-dark via-primary to-[#3B9BDD] p-12 text-white">
        <div className="flex items-center gap-2 font-heading text-[22px] font-semibold"><Gem className="h-7 w-7" /> DiamondBill</div>
        <div>
          <h1 className="font-heading text-[36px] font-semibold leading-tight text-white">Diamond Finance,<br />simplified.</h1>
          <p className="mt-4 max-w-md text-[15px] text-white/80">Inventory, certified stones, purchase &amp; sales, lab processes, accounting and reports — all in one place.</p>
        </div>
        <p className="text-[12px] text-white/60">© {new Date().getFullYear()} DiamondBill</p>
      </div>
      <div className="flex flex-1 items-center justify-center p-6">
        <div className="w-full max-w-[420px]">
          <div className="mb-8 lg:hidden flex items-center gap-2 font-heading text-[20px] font-semibold text-gray-900"><Gem className="h-6 w-6 text-primary" /> DiamondBill</div>
